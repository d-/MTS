...
#' likelihood function.  This is the same function as VMA, with the likelihood
#' function implemented in C++ for efficiency.
#'
#'
#' @usage VMACpp(da, q = 1, include.mean = T, fixed = NULL, beta=NULL,
...
#' @param include.mean A logical switch to include the mean vector. The default
#' is to include the mean vector in estimation.
...
"VMACpp" <- function(da, q = 1, include.mean = T, fixed = NULL, beta = NULL, sebeta = NULL, prelim = F, details = F, thres = 2.0) {
  ...
  if (details) {
    fit <- nlminb(
      start = ParMA, objective = LLKVMACpp, zt = da, fixed = fixed, include.mean = include.mean, q = q,
