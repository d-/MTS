...
#' Multivariate Gaussian likelihood function is used.  This is the same
#' function as VARMA, with the likelihood function implemented in C++ for
#' efficiency.
#'
#' The fixed command is used for model refinement
#'
#' @usage VARMACpp(da, p = 0, q = 0, include.mean = T, fixed = NULL, beta=NULL,
...
"VARMACpp" <- function(da, p = 0, q = 0, include.mean = T, fixed = NULL, beta = NULL, sebeta = NULL, prelim = F, details = F, thres = 2.0) {
  ...
  if (details) {
    fit <- nlminb(
      start = par, objective = LLKVARMACpp, zt = da, p = p, q = q, include.mean = include.mean, fixed = fixed,
