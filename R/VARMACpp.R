####
"VARMACpp" <- function(da,p=0,q=0,include.mean=T,fixed=NULL,prelim=F,details=F,thres=2.0){
...
# Step 5: Estimate Parameters and Compute Numerically Hessian:
   if(details){
      fit = nlminb(start = par, objective = LLKvarmaCpp, lower = lowerBounds, upper = upperBounds, control = list(trace=3))
   }
   else {
      fit = nlminb(start = par, objective = LLKvarmaCpp, lower = lowerBounds, upper = upperBounds)
   }
   ...
   for (i in 1:npar) {
      for (j in 1:npar) {
         ...
         Hessian[i, j] = (LLKvarmaCpp(x1)-LLKvarmaCpp(x2)-LLKvarmaCpp(x3)+LLKvarmaCpp(x4))/
