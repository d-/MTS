#### Multivariate Volatility Modeling Package
####
library(fGarch)
#
...
# 
# Written by Ruey Tsay in December 2012
#
if(!is.matrix(rt))rt=as.matrix(rt)
nT=dim(rt)[1]; k=dim(rt)[2]
if (m < 2) m=k+1
if(length(lb) > 2)lb=ln[1:2]
if(length(ub) > 2)ub=ub[1:2]
### Assign global variable
DCCrtn <<- rt; mused <<- m; cond.D <<- cond.dist; DCCtype <<- type
R0=cor(rt)
...
m1=optim(par,dcclike,method="L-BFGS-B",lower=c1,upper=c2,hessian=T)
...
#
# Other inputs needed:
# rt: standardized return series T-by-k
# R0: initial value of the correlation matrix
#
rt <- DCCrtn; m <- mused; cond.dist <- cond.D; loccor <- Loccor
type <- DCCtype
##
nT=dim(rt)[1]; k=dim(rt)[2]
#
...
Rt=as.matrix(Rt)
yt=matrix(rt[t,],k,1)
### The following is a direct approach
#d1=det(Rt)
#Rtinv=solve(Rt)
#P1=t(yt)%*%Rtinv
#tmp1=sum(P1%*%yt)
...
#### Obtain lower half of a matrix with diagonal elements
"Lhalf" <- function(A){
...
#### Obtain lower half of a mtrix WITHOUT the diagonal elements
"LLhalf" <- function(A){
if(!is.matrix(A))A=as.matrix(A)
Ah=NULL
k=dim(A)[2]
...
# R0: initial value of the correlation matrix
#
rt <- DCCrtn; m <- mused; loccor <- Loccor; type <- DCCtype
##
nT=dim(rt)[1]; k=dim(rt)[2]
#
R0 = cor(rt); th0=Vech(R0)
...
fit = nlminb(start = par, objective = MGAUS,
...
##ll=dmnorm(XX[t,],rep(0,k),Sigt)
##lk=lk-log(ll)
ll=dmvnorm(XX[t,],rep(0,k),sigma=Sigt,log=TRUE)
lk=lk-ll
}
...
####
#### Created on December 25, 2012 by Ruey S. Tsay
####
if(!is.matrix(rtn))rtn=as.matrix(rtn)
if(size <= 0)size=36
...
### Rectangular window approach to least squares estimation
"WLS" <- function(y,x,size,xpxi=NULL,xpy=NULL){
## Compute the recursive least squares estimates of the multiple linear 
## regression: y=beta'x+e.
######## x should include a column of 1's if constant is needed.
####
...
#### Inverse of a lower triangular matrix with 1 on the diagonal
"LinV" <- function(A){
...
########################
"MinVP" <- function(Sigma.t){
## Compute the weights of global minimum variance portfolio
##
if(!is.matrix(Sigma.t))Sigma.t = as.matrix(Sigma.t)
nT=dim(Sigma.t)[1]
k1=dim(Sigma.t)[2]
...
WGT=NULL
for (i in 1:nT){
V=matrix(Sigma.t[i,],k,k)
Vi=solve(V)
wt=apply(Vi,1,sum)
WGT=rbind(WGT,wt/sum(wt))
}
...
# 
# created by Ruey Tsay in January 2011.
...
if(include.th0){
par=c(7.0,g1,g2,th0)
#m1=optim(par,mtlikeC,method="BFGS",hessian=T)
...
cat("Lower limits: ",c1,"\n")
cat("Upper limits: ",c2,"\n")
m1=optim(par,mtlikeC,method="L-BFGS-B",lower=c1,upper=c2,hessian=T)
est=m1$par
H=m1$hessian
Hi=solve(H)
...
x3[i] = x3[i] - epsilon[i]; x3[j] = x3[j] + epsilon[j]
x4[i] = x4[i] - epsilon[i]; x4[j] = x4[j] - epsilon[j]
Hessian[i, j] = (mtlikeC(x1)-mtlikeC(x2)-mtlikeC(x3)+mtlikeC(x4))/
(4*epsilon[i]*epsilon[j])
}
}
# Step 6: Create and Print Summary Report:
...
cat("t-values:  ",est/se.coef,"\n")
##
### compute the time-varying rho.t matrices
mf = mtCopulaVol(est)
...
### Compute the time-varying rho.t based on the final estimate
"mtCopulaVol" <- function(par){
##
if(mtCopulaincludeth0){
...
nT=dim(rt)[1]; k=dim(rt)[2]
#
grp <- mtCopulagrp; ncor <- mtCopulancor
# m denotes the updating number of observations
m <- mtCopulam
#
if(mtCopulaincludeth0){
...
# Estimation of a bivariate BEKK(1,1) model
#
# rt: return series
# 
# created by Ruey Tsay in February 2011. Modified on December 24, 2012.
#
if(!is.matrix(rt))rt=as.matrix(rt)
T=dim(rt)[1]
k=dim(rt)[2]
...
fit = nlminb(start = par, objective = mlikeG,
...
x3[i] = x3[i] - epsilon[i]; x3[j] = x3[j] + epsilon[j]
x4[i] = x4[i] - epsilon[i]; x4[j] = x4[j] - epsilon[j]
Hessian[i, j] = (mlikeG(x1)-mlikeG(x2)-mlikeG(x3)+mlikeG(x4))/
(4*epsilon[i]*epsilon[j])
}
}
# Step 6: Create and Print Summary Report:
...
Prod=NULL
for (i in 1:nrow(x)){
Prod=rbind(Prod,kronecker(x[i,],x[i,]))
}
Prod
}
