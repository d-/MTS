...
#'
#' Fits a DCC model using either multivariate Gaussian or multivariate
#' Student-t innovations. Two types of DCC models are available. The first type
#' is proposed by Engle and the other is by Tse and Tsui. Both models appear in
#' the Journal of Business and Economic Statistics, 2002.
#'
#'
#' @usage dccFit(rt, type = "TseTsui", theta = c(0.90, 0.02), ub = c(0.95,
#' 0.049999), lb = c(0.4,0.00001), cond.dist = "std", df = 7, m = 0)
#' @param rt The T-by-k data matrix of k-dimensional standardized asset
#' returns. Typically, they are the standardized residuals of the command
#' dccPre.
#' @param type A logical switch to specify the type of DCC model.
#' Type="TseTsui" for Tse and Tsui's DCC model. Type = "Engle" for Engle's DCC
#' model. Default is Tse-Tsui model.
#' @param theta The initial parameter values for theta1 and theta2
#' @param ub Upper bound of parameters
#' @param lb Lower bound of parameters
#' @param cond.dist Conditional innovation distribution with std for
#' multivariate Student-t innovations.
#' @param df degrees of freedom of the multivariate Student-t innovations.
#' @param m For Tse and Tsui method only, m denotes the number of returns used
#' in local correlation matrix estimation
#' @return \item{estimates}{Parameter estimates} \item{Hessian}{Hessian matrix
#' of the estimates} \item{rho.t}{Time-varying correlation matrices.  Each row
#' contains elements of a cross-correlation matrix. }
#' @author Ruey S. Tsay
#' @seealso dccPre
#' @references Tsay (2014, Chapter 7). Multivariate Time Series Analysis with R
...
#'
#' @usage Vech(mtx)
#' @param mtx A symmetric matrix
#' @return a vector consisting of stacked elements of a symmetric matrix
#' @author Ruey S. Tsay
#' @examples
#'
#' m1 <- matrix(c(1:9), 3, 3)
...
#' Matrix constructed from output of the Vech Command.  In other words, restore
...
#'
#' @usage VechM(vec)
#' @param vec A vector representing the half-stacking of a symmetric matrix
...
#'
#' @usage dccPre(rtn, include.mean = T, p = 0, cond.dist = "norm")
#' @param rtn A T-by-k data matrix of k-dimensional asset returns
#' @param include.mean A logical switch to include a mean vector.  Deafult is
#' to include the mean.
#' @param p VAR order for the mean equation
#' @param cond.dist The conditional distribution of the innovations. Default is
#' Gaussian.
#' @return \item{marVol}{A matrix of the volatility series for each return
...
"dccPre" <- function(rtn, include.mean = T, p = 0, cond.dist = "norm") {
  ...
  for (i in 1:k) {
    ...
    Mtxest <- rbind(Mtxest, m2@fit$par)
    Mtxse <- rbind(Mtxse, m2@fit$se.coef)
...
#'
#' Use exponentially weighted moving-average method to compute the volatility
#' matrix
#'
#'
#' @usage EWMAvol(rtn, lambda = 0.96)
#' @param rtn A T-by-k data matrix of k-dimensional asset returns, assuming the
#' mean is zero
#' @param lambda Smoothing parameter. The default is 0.96. If lambda is
#' negative, then the multivariate Gaussian likelihood is used to estimate the
#' smoothing parameter.
#' @return \item{Sigma.t}{The volatility matrix with each row representing a
#' volatility matrix} \item{return}{The data} \item{lambda}{The smoothing
#' parameter lambda used}
#' @author Ruey S. Tsay
#' @references Tsay (2014, Chapter 7). Multivariate Time Series Analysis with R
#' and Financial Applications. John Wiley. Hoboken, NJ.
#' @examples
...
#'
#' @usage MCHdiag(at, Sigma.t, m = 10)
#' @param at A T-by-k matrix of residuals for a k-dimensional asset return
#' series
#' @param Sigma.t The fitted volatility matrices. The dimension is T-by-k^2
#' matrix
#' @param m The number of lags used in the tests. Default is 10.
#' @return Four test statistics and their p-values
#' @author Ruey S. Tsay
#' @references Tsay (2014, Chapter 7). Multivariate Time Series Analysis with R
#' and Financial Applications. John Wiley. Hoboken, NJ.
#' @export
...
#'
#' Use recursive least squares to perform the time-varying Cholesky
#' decomposition. The least squares estimates are then smoothed via the
#' exponentially weighted moving-average method with decaying rate 0.96.
#' University GARCH(1,1) model is used for the innovations of each linear
#' regression.
#'
#' @usage MCholV(rtn, size = 36, lambda = 0.96, p = 0)
#' @param rtn A T-by-k data matrix of a k-dimensional asset return series.
#' @param size The initial sample size used to start recursive least squares
#' estimation
#' @param lambda The exponential smoothing parameter. Default is 0.96.
#' @param p VAR order for the mean equation. Default is 0.
#' @return \item{betat}{Recursive least squares estimates of the linear
#' transformations in Cholesky decomposition} \item{bt}{The transformation
#' residual series} \item{Vol}{The volatility series of individual innovations}
...
#'
#'
#' @usage RLS(y, x, ist = 30, xpxi = NULL, xpy0 = NULL)
#' @param y data of dependent variable
#' @param x data matrix of regressors
#' @param ist initial number of data points used to start the estimation
#' @param xpxi Inverse of the X'X matrix
#' @param xpy0 Initial value of X'y.
#' @return \item{beta}{Time-varying regression coefficient estimates}
#' \item{resi}{The residual series of recursive least squares estimation}
#' @note This function is used internally, but can also be used as a command.
#' @author Ruey S. Tsay
#' @export
...
#'
#' Fits a t-copula to a k-dimensional standardized return series. The
#' correlation matrices are parameterized by angles and the angles evolve over
#' time via a DCC-type equation.
#'
#'
#' @usage mtCopula(rt, g1, g2, grp = NULL, th0 = NULL, m = 0, include.th0 =
#' TRUE, ub=c(0.95,0.049999))
#' @param rt A T-by-k data matrix of k standardized time series (after
#' univariate volatility modeling)
#' @param g1 lamda1 parameter, nonnegative and less than 1
#' @param g2 lambda2 parameter, nonnegative and satisfying lambda1+lambda2 < 1.
#' @param grp a vector to indicate the number of assets divided into groups.
#' Default means each individual asset forms a group.
#' @param th0 initial estimate of theta0
#' @param m number of lags used to estimate the local theta-angles
#' @param include.th0 A logical switch to include theta0 in estimation.
#' Default is to include.
#' @param ub Upper bound of parameters
#' @return \item{estimates}{Parameter estimates} \item{Hessian}{Hessian matrix}
#' \item{rho.t}{Cross-correlation matrices} \item{theta.t}{Time-varying angel
#' matrices}
#' @author Ruey S. Tsay
#' @references Tsay (2014, Chapter 7). Multivariate Time Series Analysis with R
#' and Financial Applications. John Wiley. Hoboken, NJ.
...
#'
#'
#' @usage SCCor(rt,end,span,grp)
#' @param rt A T-by-k data matrix of a k-dimensional time series
#' @param end The time index of the last data point to be used in computing the
#' sample correlations.
#' @param span The size of the data span used to compute the correlations.
#' @param grp A vector of group sizes. The time series in the same group are
#' pooled to compute the correlation matrix.
#' @return \item{unconCor}{Un-constrained sample correlation matrix}
#' \item{conCor}{Constrained sample correlation matrix}
#' @note This is an internal function, not intended to be a general command
#' @author Ruey S. Tsay
#' @examples
#'
...
#'
#' Perform tests to check the conditional heteroscedasticity in a time series.
#' The Ljung-Box statistics of squared series and a rank-based Ljung-Box test
#' are used.
#'
...
#'
#' @usage archTest(rt, lag = 10)
#' @param rt A scalar time series. If rt is a matrix, only the first column is
#' used.
#' @param lag The number of lags of ACF used in the Ljung-Box statistics. The
#' default is 10.
#' @return The Q-statistic and its p-value. Also, the rank-based Q statistic
#' and its p-value.
#' @author Ruey Tsay
#' @seealso MarchTest
#' @examples
...
#'
#' Perform tests to check the conditional heteroscedasticity in a vector time
#' series
#'
#' Several tests are used. First, the vector series zt is transformed into rt =
#' [t(zt)%*%Sigma-inv%*%zt]-k. The Ljung-Box statistics of rt is used to
#' perform the test. The second test is based on the ranks of the transformed
#' rt series. The third test is the multivariate Ljung-Box statistics for the
#' squared vector series zt^2. The fourth test is the multivariate Ljung-Box
#' statistics applied to the 5-percent trimmed series of the transformed series
#' rt.
#'
#' @usage MarchTest(zt, lag = 10)
#' @param zt a nT-by-k data matrix of a k-dimensional financial time series,
#' each column contains a series.
#' @param lag The number of lags of cross-correlation matrices used in the
#' tests
...
#' BEKK Model
#'
#' Estimation of a BEKK(1,1) Model for a k-dimensional time series.  Only k = 2
#' or 3 is available
#'
#'
#' @usage BEKK11(rt, include.mean = T, cond.dist = "normal", ini.estimates =
#' NULL)
#' @param rt A T-by-k data matrix of k-dimensional asset returns
#' @param include.mean A logical switch to include a constant vector in the
#' mean equation.  Default is with a constant vector.
#' @param cond.dist Conditional innovation distribution.  Only Gaussian
#' innovations are used in the current version.
#' @param ini.estimates Optional initial estimates.
#' @return \item{estimates}{Parameter estimates} \item{HessianMtx}{Hessian
#' matrix of the estimates} \item{Sigma.t}{The multivariate volatilities, each
#' row contains k-by-k elements of the volatility matrix Sigma(t)}
#' @author Ruey S. Tsay
#' @references Tsay (2014, Chapter 7)
#' @examples
#'
...
"BEKK11" <- function(rt, include.mean = T, cond.dist = "normal", ini.estimates = NULL) {
  ...
  if (k == 2) {
    if (is.null(ini.estimates)) {
      ...
      ist <- ist + 3
      A1 <- matrix(ini.estimates[(ist + 1):(ist + 4)], k, k)
      ist <- ist + 4
    ...
    if (include.mean) {
      par <- c(
        mu1 = mu[1], mu2 = mu[2], A011 = m1[1, 1], A021 = m1[1, 2], A022 = m1[2, 2], A11 = A1[1, 1], A21 = A1[2, 1], A12 = A1[1, 2], A22 = A1[2, 2],
        B11 = B1[1, 1], B21 = B1[2, 1], B12 = B1[1, 2], B22 = B1[2, 2]
      )
      c1 <- c(
        mu1 = -10 * abs(mu[1]), mu2 = -10 * abs(mu[2]), A011 = m1[1, 1] * 0.2, A021 = m1[1, 2] * 0.2, A022 = m1[2, 2] * 0.2, A11 = S, A21 = S1, A12 = S1,
        A22 = S, B11 = S, B21 = S1, B12 = S1, B22 = S
      )
      c2 <- c(
        mu1 = 10 * abs(mu[1]), mu2 = 10 * abs(mu[2]), A011 = m1[1, 1] * 1.1, A021 = m1[1, 2] * 1.1, A022 = m1[2, 2] * 1.1, A11 = 1 - S, A21 = -S1, A12 = -S1,
        A22 = 1 - S, B11 = 1 - S, B21 = -S1, B12 = -S1, B22 = 1 - S
      )
    } else {
      par <- c(
        A011 = m1[1, 1], A021 = m1[1, 2], A022 = m1[2, 2], A11 = A1[1, 1], A21 = A1[2, 1], A12 = A1[1, 2], A22 = A1[2, 2],
        B11 = B1[1, 1], B21 = B1[2, 1], B12 = B1[1, 2], B22 = B1[2, 2]
      )
      c1 <- c(
        A011 = m1[1, 1] * 0.2, A021 = m1[1, 2] * 0.2, A022 = m1[2, 2] * 0.2, A11 = S, A21 = S1, A12 = S1,
        A22 = S, B11 = S, B21 = S1, B12 = S1, B22 = S
      )
      c2 <- c(
        A011 = m1[1, 1] * 1.1, A021 = m1[1, 2] * 1.1, A022 = m1[2, 2] * 1.1, A11 = 1 - S, A21 = -S1, A12 = -S1,
        A22 = 1 - S, B11 = 1 - S, B21 = -S1, B12 = -S1, B22 = 1 - S
      )
    }
    ### end of if(k==2)
  ...
  if (k == 3) {
    if (is.null(ini.estimates)) {
      ...
      ist <- ist + 6
      A1 <- matrix(ini.estimates[(ist + 1):(ist + 9)], k, k)
      ist <- ist + 9
    ...
    if (include.mean) {
      par <- c(
        mu1 = mu[1], mu2 = mu[2], mu3 = mu[3], A011 = m1[1, 1], A021 = m1[1, 2], A031 = m1[1, 3], A022 = m1[2, 2], A032 = m1[2, 3], A033 = m1[3, 3], A11 = A1[1, 1], A21 = A1[2, 1], A31 = A1[3, 1], A12 = A1[1, 2], A22 = A1[2, 2], A32 = A1[3, 2], A13 = A1[1, 3], A23 = A1[2, 3], A33 = A1[3, 3],
        B11 = B1[1, 1], B21 = B1[2, 1], B31 = B1[3, 1], B12 = B1[1, 2], B22 = B1[2, 2], B32 = B1[3, 2], B13 = B1[1, 3], B23 = B1[2, 3], B33 = B1[3, 3]
      )
      c1 <- c(
        mu1 = -10 * abs(mu[1]), mu2 = -10 * abs(mu[2]), mu3 = -10 * abs(mu[3]), A011 = m1[1, 1] * 0.2, A021 = m1[1, 2] * 0.2, A031 = m1[1, 3] * 0.2, A022 = m1[2, 2] * 0.2, A032 = m1[2, 3] * 0.2, A033 = m1[3, 3] * 0.2, A11 = S, A21 = S1, A31 = S1, A12 = S1,
        A22 = S, A32 = S1, A13 = S1, A23 = S1, A33 = S, B11 = S, B21 = S1, B31 = S1, B12 = S1, B22 = S, B32 = S1, B13 = S1, B23 = S1, B33 = S
      )
      c2 <- c(
        mu1 = 10 * abs(mu[1]), mu2 = 10 * abs(mu[2]), mu3 = 10 * abs(mu[3]), A011 = m1[1, 1] * 1.1, A021 = m1[1, 2] * 1.1, A031 = m1[1, 3] * 1.1, A022 = m1[2, 2] * 1.1, A032 = m1[2, 3] * 1.1, A033 = m1[3, 3] * 1.1, A11 = 1 - S, A21 = -S1, A31 = -S1, A12 = -S1,
        A22 = 1 - S, A32 = -S1, A13 = -S1, A23 = -S1, A33 = 1 - S, B11 = 1 - S, B21 = -S1, B31 = -S1, B12 = -S1, B22 = 1 - S, B32 = -S1, B13 = -S1, B23 = -S1, B33 = 1 - S
      )
    } else {
      par <- c(
        A011 = m1[1, 1], A021 = m1[1, 2], A031 = m1[1, 3], A022 = m1[2, 2], A032 = m1[2, 3], A033 = m1[3, 3], A11 = A1[1, 1], A21 = A1[2, 1], A31 = A1[3, 1], A12 = A1[1, 2], A22 = A1[2, 2], A32 = A1[3, 2], A13 = A1[1, 3], A23 = A1[2, 3], A33 = A1[3, 3],
        B11 = B1[1, 1], B21 = B1[2, 1], B31 = B1[3, 1], B12 = B1[1, 2], B22 = B1[2, 2], B32 = B1[3, 2], B13 = B1[1, 3], B23 = B1[2, 3], B33 = B1[3, 3]
      )
      c1 <- c(
        A011 = m1[1, 1] * 0.2, A021 = m1[1, 2] * 0.2, A031 = m1[1, 3] * 0.2, A022 = m1[2, 2] * 0.2, A032 = m1[2, 3] * 0.2, A033 = m1[3, 3] * 0.2, A11 = S, A21 = S1, A31 = S1, A12 = S1,
        A22 = S, A32 = S1, A13 = S1, A23 = S1, A33 = S, B11 = S, B21 = S1, B31 = S1, B12 = S1, B22 = S, B32 = S1, B13 = S1, B23 = S1, B33 = S
      )
      c2 <- c(
        A011 = m1[1, 1] * 1.1, A021 = m1[1, 2] * 1.1, A031 = m1[1, 3] * 1.1, A022 = m1[2, 2] * 1.1, A032 = m1[2, 3] * 1.1, A033 = m1[3, 3] * 1.1, A11 = 1 - S, A21 = -S1, A31 = -S1, A12 = -S1,
        A22 = 1 - S, A32 = -S1, A13 = -S1, A23 = -S1, A33 = 1 - S, B11 = 1 - S, B21 = -S1, B31 = -S1, B12 = -S1, B22 = 1 - S, B32 = -S1, B13 = -S1, B23 = -S1, B33 = 1 - S
      )
    }
    ### end of if(k==3)
  ...
  fit <- nlminb(
    start = par, objective = mlikeG, RTN = RTN, include.mean = include.mean,
    lower = c1, upper = c2
